# Brief
This project builds a client/server network. 
1. A dictionary is created, populated, serialised and sent to a server.
2. A text file is created and sent to a server.
3. It is coded using socket programming.
# Results
* The client can set the pickling format to binary, JSON and XML; and encrypt the text file.
* The server has a configurable option to print the contents of the sent items to the screen and/or to a file; and to decrypt files.
* The built network works on both, the separate and same machine.
# Prereq
``` 
python (tested on 3.11)
 ```
# Setup 
```
pip install -r requirements.txt
```
# Testing
# Brief
This project builds a client/server network. 
1. A dictionary is created, populated, serialised and sent to a server.
2. A text file is created and sent to a server.
3. It is coded using socket programming.
# Results
* The client can set the pickling format to binary, JSON and XML; and encrypt the text file.
* The server has a configurable option to print the contents of the sent items to the screen and/or to a file; and to decrypt files.
* The built network works on both, the separate and same machine.
# Prereq
``` 
python (tested on 3.11)
 ```
# Setup 
```
pip install -r requirements.txt
```
# Testing
```
pip install -m unittest
```
assertIsInstance is showcased
# Contributing
Patrick Bracebridge;
Sanet Shepperson;
Natalya Smith.

Pull requests are welcome. 
Please update tests as appropriate. 
# License
This project is MIT licensed.
# References
See notes within code. 

# Contributing
Patrick Bracebridge;
Sanet Shepperson;
Natalya Smith.

Pull requests are welcome. 
Please update tests as appropriate. 
# License
This project is MIT licensed.
# References
See notes within code. 
